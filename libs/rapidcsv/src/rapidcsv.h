...
namespace rapidcsv
{
  ...
  private:
    ...
    void ParseCsv(std::istream& pStream, std::streamsize p_FileLength)
    {
          ...
          else if (buffer[i] == '\n')
          {
            ...
            else
            {
              ...
              else
              {
                ...
                std::cout << "Parsed row: "; // Добавить эту строку
                for (const auto& r : row) std::cout << r << ", "; // Добавить эту строку
                std::cout << std::endl; // Добавить эту строку
      ...
      if (!row.empty())
      {
        std::cout << "Final row before adding to mData: "; // Добавить эту строку
        for (const auto& r : row) std::cout << r << ", "; // Добавить эту строку
        std::cout << std::endl; // Добавить эту строку
